...
const generatePoolGraphData = async (assets: PoolAssets[], numberOfDays: number, initUnitCost: number, fileName: string) => {
	try {
		// const possibleDateDetails = await mockPoolStartDate(assets);
		// if (!possibleDateDetails) {
		//     throw new Error("Could not calculate start date for the pool");
		...
		// const startDate = dayjs(possibleDateDetails.startDate).add(30, "day");
		const startDate = dayjs().subtract(numberOfDays, "day").hour(0).minute(0).second(0).millisecond(0).add(dayjs().utcOffset(), "minute");
		...
		for (let i = numberOfDays, j = 1; i > 1108; i--, j++) {
			const diffInUTC = dayjs().utcOffset();
			const forDate = dayjs().subtract(i, "day").hour(0).minute(0).second(0).millisecond(0).add(diffInUTC, "minute");

			const calculatedDetails = await calculationsAtADate(assets, poolBalances, forDate.toDate(), totalShareBalance);
			...
			resultData.push({
				...
				// poolBalances,
				costPerUnit: calculatedDetails.newCostPerUnit.mul(10 ** 18).toFixed(0),
				// totalUnitsAllocated: totalShareBalance.toFixed(),
		...
		console.log(`For Mock Pool of ${n} years CAGR is ${cagrForSIP}`);
...
export default generatePoolGraphData;
